...
-- 사용자 계정 생성
CREATE USER member_hjh IDENTIFIED BY member1234;

-- 생성한 사용자 계정에 권한 부여
GRANT RESOURCE, CONNECT TO member_hjh;

-- 테이블 스페이스 할당
ALTER USER member_hjh DEFAULT TABLESPACE SYSTEM
...
MEMBER_NM ='바꾼이름', --입력
MEMBER_GENDER ='F' -- 입력
WHERE MEMBER_NO = 1; -- loginMember.getMember.No();
...
INSERT INTO BOARD
VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 제목5', '샘플 내용 5 입니다.',
...
--
	CASE 
		WHEN SYSDATE - CREATE_DT < 1/24/60 -- 현재시간과 게시글 작성 시간의 차이가 1분 미만인 경우
		THEN FLOOR((SYSDATE - CREATE_DT)*24*60*60 ) || '초 전'
		WHEN SYSDATE - CREATE_DT < 1/24 -- 현재시간과 게시글 작성 시간의 차이가 1시간 미만인 경우
		THEN FLOOR((SYSDATE - CREATE_DT)*24*60 ) || '분 전'
		WHEN SYSDATE - CREATE_DT < 1 -- 현재시간과 게시글 작성 시간의 차이가 1일 미만인 경우
